...
class MachineExcelWriterTest(p_site_id: String,
  ...
  //asset save Obj 배열
  var saveAssetList: mutable.ArrayBuffer[MachineExcelSaveConfigTest] = new mutable.ArrayBuffer[MachineExcelSaveConfigTest]()
  ...
  val jdbcHelper: JdbcHelperTest = new JdbcHelperTest()
  ...
  override def process(record: Row): Unit = {
    ...
    if(!matching){
      // asset save Obj 생성
      val saveAssetObj: MachineExcelSaveConfigTest =  new MachineExcelSaveConfigTest(site_id, mongodb_uri, mdb_name, colNm, lotNum, w_asset_list, w_tag_list);
...
//        .config("spark.executor.memory", "1GB")
      ...
      //저장 객체 리스트 초기화
      saveAssetList = new mutable.ArrayBuffer[MachineExcelSaveConfigTest]();
    }
  }
}
