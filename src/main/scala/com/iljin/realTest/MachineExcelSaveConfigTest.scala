...
class MachineExcelSaveConfigTest(p_site_id: String,
                             p_uri: String,
                             p_dbName: String,
  ...
  val writeConfig: WriteConfig = WriteConfig(Map("uri" -> (mongodbURI + "/" + dbName + "." + collectionName +"_temp")))
  ...
  def initMariaDb() = {
    ...
    if (modelingYn && conn == null) {
      ...
      sql = "insert into " + collectionName + "_temp ("
